public class LinkedList {
	 ...
	 */
	public int size(){
		int count = 1;//Counter starts at 1 to include headNode.
		if (headNode == null)
			return 0;
		else {
			for(Node n = headNode ; n.getNextNode() != null ; n = n.getNextNode()) {
				count++;
			}//Loop traverses through loop and increments counter
			return count;
		}
	}
	...
	public int total(){
		...
		if (headNode == null)
			...
			sum += headNode.getValue();
			while (headNode.getNextNode() != null) {
				headNode = headNode.getNextNode();
				sum += headNode.getValue();
			}//Adds headNodes value to sum
			return sum;
